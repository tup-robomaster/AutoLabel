...
def autoLabelByStream(teacher, cap,debug=True):
    """使用视频流自动标注
    Args:
        teacher (Teacher): Teacher类
    ...
    while(True):
        ...
        for det in result:
            ...
            if (det.id == 6):
                target.cls = 9 * det.color + 7
            else:
                target.cls = 9 * det.color + det.id
                target.apex = np.array(det.pts)
...
def autoLabelByDaheng(teacher, exp, max_delta_exp):
    """使用大恒相机自动标注
    Args:
        teacher (Teacher): Teacher类
        exp (int): 平均曝光
...
def autoLabelByImgs(teacher, img_list):
    # -----------------------初始化-----------------------------
    cnt = 0
    path = "data/"
    print("正在进行自动标注 ... ")
...
def main():
    ...
    parser.add_argument("--type", "-t", type=str, help="输入类型,支持daheng,local_imgs, local_video, stream.")
    parser.add_argument("--input_dir", "-i", help="目标数据位置.")
    parser.add_argument("--video", help="目标视频位置.")
    args = parser.parse_args()
    # --------------------------初始化变量-----------------------------
    default_img_format = [".jpg", ".png", ".jpeg"]
    teacher = Teacher("model-opt.onnx")
    mean_exposure = 7500
    ...
    if (using_daheng):
        autoLabelByDaheng(teacher, mean_exposure, max_delta_exposure)
    elif (using_local_imgs):
        autoLabelByImgs(teacher, img_list)
    elif (using_local_video or using_stream):
        autoLabelByStream(teacher, cap)
